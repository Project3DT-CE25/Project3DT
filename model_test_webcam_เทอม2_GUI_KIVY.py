...
# # python model_test_webcam.py --modeldir=TFLite_model // for run
            ...
            for i in range(len(scores)):
                if ((scores[i] > min_conf_threshold) and (scores[i] <= 1.0)):
                    ...
                    if object_name == "NoMask":
                        self.main_screen.ids.shownNomask.opacity = 1
                        cv2.rectangle(frame, (xmin,ymin), (xmax,ymax), (0, 0, 210), 2) 
                    else:
                        self.main_screen.ids.shownNomask.opacity = 0
